...
const SalesPage: React.FC = () => {
  ...
  const handleAddItem = () => {
    ...
    setNewSale(prev => ({
      ...prev,
      items: [...prev.items, { product_id: parseInt(currentItem.product_id), quantity: parseInt(currentItem.quantity) }],
    }));
    setCurrentItem({ product_id: '', quantity: '1' });
  };
  
  const getProductName = (id: number) => products.find(p => p.product_id === id)?.name || 'Desconhecido';

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newSale.customer_id || newSale.items.length === 0 || !newSale.payment_method_id) {
      alert('Cliente, Itens de Venda e Método de Pagamento são obrigatórios.');
      return;
    }
    const salePayload: SalePayload = {
        ...
        items: newSale.items,
        payment_method_id: parseInt(newSale.payment_method_id)
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="p-4 border rounded border-slate-300 dark:border-slate-600">
            ...
            <div className="flex gap-2 items-center mb-2">
              ...
              <button type="button" onClick={handleAddItem} className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-3 rounded">+</button>
            </div>
